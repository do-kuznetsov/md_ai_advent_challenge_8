...
import com.sibgear.deepseek.chat.data.deepseek.internal.mapper.toAssistantInvariants
...
class DeepSeekAssistantProfileService(
    ...
    override suspend fun updateInvariants(
        ...
        val content = json.decodeFromString<DeepSeekChatCompletionResponse>(body)
            ...
            ?.content
            ?.takeIf { it.isNotBlank() }
            ?: error("DeepSeek вернул пустой список инвариантов.")

        return content.toAssistantInvariants(json)
            ?: error("DeepSeek вернул инварианты в неожиданном формате.")
    }
