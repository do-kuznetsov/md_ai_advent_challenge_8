...
class KtorAiRepository(
    private val credentials: AiProviderCredentials,
    ...
    private suspend fun sendNonStreamingCompletion(request: AiRequestData): CompletionResult {
        val response = client.post(request.model.provider.chatCompletionsUrl) {
            bearerAuth(request.model.provider.apiKey(credentials))
    ...
    private suspend fun sendOpenRouterStreamingCompletion(request: AiRequestData): CompletionResult {
        val response = client.post(AiProvider.OpenRouter.chatCompletionsUrl) {
            bearerAuth(AiProvider.OpenRouter.apiKey(credentials))
            header(HttpHeaders.ContentType, ContentType.Application.Json.toString())
    ...
    override suspend fun loadOpenRouterModels(): List<AiModel> {
...
private fun AiProvider.apiKey(credentials: AiProviderCredentials): String =
    when (this) {
        AiProvider.DeepSeek -> credentials.deepSeekApiKey
        AiProvider.OpenRouter -> credentials.openRouterAiKey
    }
